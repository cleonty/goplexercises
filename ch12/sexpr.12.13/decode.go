...
func readList(lex *lexer, v reflect.Value) {
	...
	case reflect.Struct: // ((name value) ...)
		for !endList(lex) {
			...
			name := lex.text()
			lex.next()
			read(lex, fieldByTagOrName(v, name))
			lex.consume(')')
		}
...
func fieldByTagOrName(v reflect.Value, tag string) reflect.Value {
	for i := 0; i < v.NumField(); i++ {
		field := v.Type().Field(i)
		fieldTag := field.Tag
		name := fieldTag.Get("sexpr")
		if name == "" {
			name = field.Name
		}
		if name == tag {
	...
	panic(fmt.Sprintf("field %q wasn't found", tag))
}
